...
export default function Detail({date,n_page,n_cours,author,language,category,loading}) {
  ...
  return (
    <div className="mt-5">
        <div className="flex mt-1.5 sm:mt-3 flex-col sm:flex-row gap-1.5 sm:gap-0">
            ...
            <div className="w-full sm:w-1/2 flex items-center gap-1"><Person width={20} height={20} color={'#3b82f6'}/> <h4 className="mb-0 font-medium text-sm">Publié par :</h4> {!loading ? <p style={{color :'#11843c'}} className="mb-0 font-medium text-sm">{author}</p> : <div className="rounded py-1 bg-gray-200 animate-pulse w-2/12"></div>}</div>
        </div>
        <div className="flex mt-1.5 sm:mt-3 flex-col sm:flex-row gap-1.5 sm:gap-0">
